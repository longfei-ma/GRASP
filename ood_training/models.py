...
from torch_geometric.nn import GCNConv, SGConv, GATConv, JumpingKnowledge, APPNP, GCN2Conv, MessagePassing, GATv2Conv, GINConv, SAGEConv
...
class GAT(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2,
        ...
        self.convs.append(
            ...
            GATv2Conv(in_channels, hidden_channels, heads=heads, concat=True, add_self_loops=add_self_loops)
            )
        ...
        for _ in range(num_layers - 2):
            ...
            self.convs.append(
                    ...
                    GATv2Conv(hidden_channels*heads, hidden_channels, heads=heads, concat=True, add_self_loops=add_self_loops) 
                    ) 
            self.bns.append(nn.BatchNorm1d(hidden_channels*heads))

        self.convs.append(
            # GATConv(hidden_channels*heads, out_channels, heads=heads, concat=False, add_self_loops=add_self_loops)
            GATv2Conv(hidden_channels*heads, out_channels, heads=heads, concat=False, add_self_loops=add_self_loops)
            )
...
class GIN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2,
        ...
        self.convs = nn.ModuleList()
        nnn = Sequential(Linear(in_channels, hidden_channels), ReLU(), Linear(hidden_channels, hidden_channels))
        conv = GINConv(nnn)
        ...
        for _ in range(num_layers - 1):
            nnn = Sequential(Linear(hidden_channels, hidden_channels), ReLU(), Linear(hidden_channels, hidden_channels))
            conv = GINConv(nnn)
            self.convs.append(conv) 
            self.bns.append(nn.BatchNorm1d(hidden_channels))
